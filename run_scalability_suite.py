...
# ── deterministic compute counters (machine-independent cost of the hierarchy) ──
# Counted per-layout (reset in run_layout, read at the end). These reflect the true
# algorithmic work, unlike wall-time which is noisy and machine-dependent.
_COMPUTE = {"master_fwd_calls": 0, "master_node_evals": 0, "agent_node_evals": 0}


def _reset_compute() -> None:
    _COMPUTE["master_fwd_calls"] = 0
    _COMPUTE["master_node_evals"] = 0
    _COMPUTE["agent_node_evals"] = 0
...
def _proc_cpu_seconds() -> float:
    ...
    Machine-independent measure of actual compute, unlike instantaneous CPU%."""
...
class _PeakRSSSampler:
    """Samples resident memory in a background thread and tracks the peak (MB)."""
    ...
    def start(self) -> None:
        if _PROC is None:
            return
        import threading
        self._peak = self._rss_mb()

        def _loop():
            while not self._stop:
                self._peak = max(self._peak, self._rss_mb())
    ...
    def stop(self) -> float:
        ...
        # final sample in case the peak happened right at the end
        self._peak = max(self._peak, self._rss_mb())
        return self._peak
...
def master_embeddings_batch(master_model, inputs: list[np.ndarray], proto_exp, deterministic: bool) -> list[np.ndarray]:
    ...
    if not inputs:
        return []
    # Each packed input is ONE master-node evaluation (an LM, an intermediate master,
    # or the GM); a call is one batched forward pass over all of them.
    _COMPUTE["master_fwd_calls"] += 1
    _COMPUTE["master_node_evals"] += len(inputs)
        ...
        for m, c in enumerate(cells):
            ...
            for g, grp in enumerate(c.groups):
                ...
                for i in grp:
                    ...
                    owners.append(m)
        if obs_all:
            _COMPUTE["agent_node_evals"] += len(obs_all)
...
def run_layout(
    ...
    # ── compute-cost instrumentation (deterministic counters + CPU + peak RAM) ──
    _reset_compute()
    ...
    out["cpu_sec"] = round(_proc_cpu_seconds() - cpu0, 2)
    out["peak_rss_mb"] = round(peak_rss_mb, 1)
    out["master_node_evals"] = int(_COMPUTE["master_node_evals"])
    out["agent_node_evals"] = int(_COMPUTE["agent_node_evals"])
    out["master_fwd_calls"] = int(_COMPUTE["master_fwd_calls"])
    out["network_calls_total"] = int(_COMPUTE["master_node_evals"] + _COMPUTE["agent_node_evals"])
...
def write_tables(results: list[dict[str, Any]], out_root: str) -> None:
    ...
    cols = ["M_local_masters", "K_agents_per_master", "N_total_agents", "n_intersections",
            ...
            "crash_rate_any_episode_pct", "mean_steps", "total_env_steps",
            "master_node_evals", "agent_node_evals", "network_calls_total",
            "cpu_sec", "peak_rss_mb", "wall_time_sec", "n_scenarios"]
        ...
        for r in res:
            for cond in conds:
                ...
                w.writerow([
                    ...
                    f"{cc['mean_steps']:.1f}", r.get("total_env_steps", 0),
                    r.get("master_node_evals", 0), r.get("agent_node_evals", 0),
                    r.get("network_calls_total", 0), r.get("cpu_sec", 0.0),
                    r.get("peak_rss_mb", 0.0), r.get("wall_time_sec", 0.0),
                    cc.get("n_scenarios", r.get("n_scenarios", 0)),
                ])
...
def make_compute_plot(results: list[dict[str, Any]], out_png: str) -> None:
    """Compute cost vs scale. Wall-time is intentionally NOT plotted (it is noisy and
    machine-dependent). Instead we report deterministic / machine-robust measures,
    each as its OWN image:
      * ``*_calls.png``  — neural-network forward evaluations (master nodes + agents)
      * ``*_cpu.png``    — CPU-seconds consumed by the process
      * ``*_steps.png``  — total environment steps simulated
      * ``*_memory.png`` — peak resident memory (RAM)
    """
    res = sorted(results, key=lambda r: r["total_agents"])
    xs = [r["total_agents"] for r in res]
    n_cond = len(_present_conditions(results))
    suffix = f"(all {n_cond} conditions)"
    base, ext = os.path.splitext(out_png)
    ext = ext or ".png"

    def _single(ys, title, ylabel, color, fname, marker="o", second=None):
        fig, ax = plt.subplots(figsize=(8, 5.5))
        ax.plot(xs, ys, marker=marker, lw=2, color=color, label=ylabel)
        if second is not None:
            ys2, lbl2, col2 = second
            ax.plot(xs, ys2, marker="s", lw=2, color=col2, label=lbl2)
            ax.legend(fontsize=9)
        ax.set_xlabel("Total agents (N)")
        ax.set_ylabel(ylabel, color=color if second is None else "black")
        if second is None:
        ...
        ax.set_title(title)
        fig.tight_layout()
        rps.safe_figure_save_png(fig, f"{base}_{fname}{ext}", dpi=170)
        plt.close(fig)

    # 1) Forward evaluations: master nodes + agents (deterministic).
    master_ev = [r.get("master_node_evals", 0) for r in res]
    agent_ev = [r.get("agent_node_evals", 0) for r in res]
    fig, ax = plt.subplots(figsize=(8, 5.5))
    ax.plot(xs, agent_ev, marker="o", lw=2, color="#2E7D32", label="Agent forward evals")
    ax.plot(xs, master_ev, marker="s", lw=2, color="#6A1B9A", label="Master-node evals (LM+mid+GM)")
    ax.set_xlabel("Total agents (N)")
    ax.set_ylabel(f"Network forward evaluations {suffix}")
    ax.set_xticks(xs)
    ax.grid(True, alpha=0.3)
    ax.legend(fontsize=9)
    ax.set_title("Network calls vs scale (deterministic)")
    fig.tight_layout()
    rps.safe_figure_save_png(fig, f"{base}_calls{ext}", dpi=170)
    plt.close(fig)

    # 2) CPU-seconds.
    _single([r.get("cpu_sec", 0.0) for r in res],
            "CPU cost vs scale", f"CPU-seconds {suffix}", "#1565C0", "cpu")
    # 3) Environment steps.
    _single([r.get("total_env_steps", 0) for r in res],
            "Simulation work vs scale", f"Total environment steps {suffix}", "#C62828", "steps", marker="s")
    # 4) Peak memory.
    _single([r.get("peak_rss_mb", 0.0) for r in res],
            "Memory footprint vs scale", "Peak resident memory (MB)", "#EF6C00", "memory", marker="D")
