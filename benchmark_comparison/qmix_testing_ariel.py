...
import highway_env  # Required for intersection-v0 environment
...
class QMIXwD:
    ...
    def pretrain_step(self, batch_size=32, lambda_demo=1.0, lambda_td_loss=1.0, lambda_l2=1e-4):
        """
        Pre-training step with demonstration data

        Loss: L_tot = λ1*L_demo + λ2*L_TD(λ) + λ3*L_L2
        """
        if len(self.demo_buffer) < batch_size:
            return None

        # Sample episodes from demonstration buffer
        ...
        for episode in episodes:
            ...
            for t, transition in enumerate(episode):
                ...
                # TD(λ) loss
                td_loss = F.mse_loss(q_tot, target_tensor.unsqueeze(1))

                # Supervised margin loss for each agent
                demo_loss = 0.0
                for i, obs_tensor in enumerate(obs_tensors):
                    q_values = self.q_network(obs_tensor)
                    max_q = q_values.max(dim=1)[0]

                    # Margin function: l(a_demo, a) = 0 if a == a_demo, else positive constant
                    margin = 0.8
                    action_q = q_values[0, actions_tensor[i]]

                    # L_demo = [Q(τ,a) - (max_a Q(τ,a) - l(a_demo,a))]^2
                    # This encourages Q(τ, a_demo) to be close to max_a Q(τ,a)
                    target_q = max_q - margin
                    demo_loss += F.mse_loss(action_q, target_q.detach())
                ...
                for param in self.mixing_network.parameters():
                    l2_loss += torch.sum(param ** 2)
                ...
                demo_loss_total += demo_loss.item()
                td_loss_total += td_loss.item()
        ...
        return {
            ...
            'demo_loss': demo_loss_total / n_transitions,
            'td_loss': td_loss_total / n_transitions,
            'l2_loss': l2_loss_total / n_transitions
        }
...
class IntersectionWrapper:
    """
    Wrapper for highway-env intersection with scenario management.
    Provides MARL interface with proper observation/state formatting.
    """

    def __init__(self, scenario_manager, n_agents=2):
        ...
        import highway_env  # Import highway_env to register environments
        self.env = gym.make('intersection-v0', render_mode=None, config=config)

        print(f"[IntersectionWrapper] Environment created")

    def reset(self, scenario_idx=None):
        """
        Reset environment with a specific or random scenario

        Args:
            scenario_idx: specific scenario index (0-99), or None for random
        Returns:
        ...
        # IMPORTANT: Apply scenario by manually positioning vehicles
        # This is a simplified version - highway-env scenarios are complex
        # For now, we'll use the random initialization but with awareness
        # that proper scenario application would require deeper highway-env integration

        # Note: Full scenario application would require:
        # 1. Setting vehicle routes via plan_route_to()
        # 2. Manually positioning vehicles at specific lane locations
        # 3. This requires access to env.road.vehicles which may not be
        #    available through the gymnasium wrapper

        # Extract observations for each controlled vehicle
        if isinstance(obs_raw, np.ndarray):
            if len(obs_raw.shape) == 1:
                n_features = 4
                n_vehicles = len(obs_raw) // n_features
                obs_raw = obs_raw.reshape(n_vehicles, n_features)

        # Get observations for controlled agents (first n_agents)
        obs = []
        ...
        # Global state: all vehicles' states
    ...
    def step(self, actions):
        ...
        Returns:
            next_obs: list of observations for each agent
            next_state: global state
            reward: shared reward
            done: episode termination flag
            truncated: episode truncation flag
            info: additional information
        """
        # Convert list of actions to tuple for highway-env
...
def train_qmixwd(
    ...
    for episode in range(n_episodes):
        ...
        # Record results
        success = not crashed
        results['train_success_rates'].append(1 if success else 0)
        ...
        if (episode + 1) % 100 == 0:
            recent_sr = np.mean(results['train_success_rates'][-100:])
            recent_reward = np.mean(results['train_rewards'][-100:])

            elapsed = time.time() - phase3_start
            remaining = (elapsed / (episode + 1)) * (n_episodes - episode - 1)

            print(f"Episode {episode + 1}/{n_episodes} | "
                  f"Success: {recent_sr:.2%} | "
...
def save_results(train_results, eval_results, save_dir):
    """
    Save training curves, evaluation results, and summary statistics
    ...
    # Subplot 1: Training success rate
    plt.subplot(2, 3, 1)
    window = 100
    if len(train_results['train_success_rates']) >= window:
        train_sr_smoothed = np.convolve(
            train_results['train_success_rates'],
            np.ones(window) / window,
            mode='valid'
        )
        plt.plot(train_sr_smoothed, label='Success Rate (100-ep avg)', linewidth=2)
    else:
        plt.plot(train_results['train_success_rates'], label='Success Rate', linewidth=2)

    plt.xlabel('Episode', fontsize=12)
    plt.ylabel('Success Rate', fontsize=12)
    plt.title('Training Success Rate', fontsize=14, fontweight='bold')
    plt.grid(True, alpha=0.3)
    plt.legend()

    # Subplot 2: Training reward
    plt.subplot(2, 3, 2)
    if len(train_results['train_rewards']) >= window:
        train_reward_smoothed = np.convolve(
            train_results['train_rewards'],
            np.ones(window) / window,
            mode='valid'
        )
        plt.plot(train_reward_smoothed, label='Reward (100-ep avg)', linewidth=2, color='orange')
    else:
        plt.plot(train_results['train_rewards'], label='Reward', linewidth=2, color='orange')
    ...
    # Subplot 3: Pre-training loss
    plt.subplot(2, 3, 3)
    if len(train_results['pretrain_losses']) > 0:
        plt.plot(train_results['pretrain_losses'], linewidth=2, color='red')
        plt.xlabel('Pre-training Step', fontsize=12)
        plt.ylabel('Loss', fontsize=12)
        plt.title('Pre-training Loss', fontsize=14, fontweight='bold')
        plt.grid(True, alpha=0.3)

    # Subplot 4: Online training loss
    plt.subplot(2, 3, 4)
    if len(train_results['train_losses']) > 0:
        plt.plot(train_results['train_losses'], linewidth=2, color='green')
        plt.xlabel('Training Step', fontsize=12)
        plt.ylabel('Loss', fontsize=12)
        plt.title('Online Training Loss', fontsize=14, fontweight='bold')
        plt.grid(True, alpha=0.3)

    # Subplot 5: Success rate comparison
    plt.subplot(2, 3, 5)
    train_final_sr = np.mean(train_results['train_success_rates'][-1000:]) if len(
        train_results['train_success_rates']) >= 1000 else np.mean(train_results['train_success_rates'])
    eval_sr = np.mean(eval_results['success_rates'])

    bars = plt.bar(['Training\n(last 1000 eps)', 'Evaluation\n(100 scenarios)'],
                   [train_final_sr, eval_sr],
                   color=['steelblue', 'coral'],
                   alpha=0.7)

    plt.ylabel('Success Rate', fontsize=12)
    plt.title('Final Success Rates', fontsize=14, fontweight='bold')
    plt.ylim([0, 1])
    ...
    # Subplot 6: Episode length distribution
    plt.subplot(2, 3, 6)
    if len(eval_results['episode_lengths']) > 0:
        plt.hist(eval_results['episode_lengths'], bins=20, color='purple', alpha=0.7, edgecolor='black')
        plt.xlabel('Episode Length', fontsize=12)
        plt.ylabel('Frequency', fontsize=12)
        plt.title('Evaluation Episode Lengths', fontsize=14, fontweight='bold')
        plt.grid(True, alpha=0.3, axis='y')

    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, 'training_results.png'), dpi=300, bbox_inches='tight')
    print(f"[Results] Plots saved to {save_dir}/training_results.png")
    plt.close()
    ...
    summary = {
        'training': {
            'total_episodes': len(train_results['train_success_rates']),
            'final_success_rate': float(train_final_sr),
            'final_avg_reward': float(np.mean(train_results['train_rewards'][-1000:])) if len(
                train_results['train_rewards']) >= 1000 else float(np.mean(train_results['train_rewards'])),
            'final_avg_episode_length': float(np.mean(train_results['episode_lengths'][-1000:])) if len(
                train_results['episode_lengths']) >= 1000 else float(np.mean(train_results['episode_lengths'])),
        },
        'evaluation': {
            'n_scenarios': len(eval_results['success_rates']),
            'success_rate': float(np.mean(eval_results['success_rates'])),
            'n_successes': int(np.sum(eval_results['success_rates'])),
            'avg_reward': float(np.mean(eval_results['rewards'])),
            'avg_episode_length': float(np.mean(eval_results['episode_lengths'])),
            'avg_travel_time_successful': float(np.mean(eval_results['travel_times'])) if len(
                eval_results['travel_times']) > 0 else 0.0,
        }
    }
    ...
    print(f"[Results] Summary saved to {save_dir}/summary.json")
    ...
    np.savez(
        ...
        train_success_rates=train_results['train_success_rates'],
        train_rewards=train_results['train_rewards'],
        train_episode_lengths=train_results['episode_lengths'],
        pretrain_losses=train_results['pretrain_losses'],
        train_losses=train_results['train_losses'],
        eval_success_rates=eval_results['success_rates'],
        eval_rewards=eval_results['rewards'],
        eval_episode_lengths=eval_results['episode_lengths'],
        eval_travel_times=eval_results['travel_times']
    )

    print(f"[Results] Raw data saved to {save_dir}/raw_data.npz")
    ...
    # Training CSV
    with open(os.path.join(save_dir, 'training_success_rate.csv'), 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Episode', 'Success', 'Reward', 'Episode_Length'])
        for i, (success, reward, length) in enumerate(zip(
                train_results['train_success_rates'],
                train_results['train_rewards'],
                train_results['episode_lengths']
        )):
            writer.writerow([i + 1, int(success), reward, length])

    print(f"[Results] Training CSV saved to {save_dir}/training_success_rate.csv")

    # Evaluation CSV
    with open(os.path.join(save_dir, 'evaluation_success_rate.csv'), 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['Scenario_ID', 'Success', 'Reward', 'Episode_Length'])
        for i, (success, reward, length) in enumerate(zip(
                eval_results['success_rates'],
                eval_results['rewards'],
                eval_results['episode_lengths']
        )):
            writer.writerow([i, int(success), reward, length])

    print(f"[Results] Evaluation CSV saved to {save_dir}/evaluation_success_rate.csv")
    ...
    print("=" * 70)
    print(f"\nTRAINING ({summary['training']['total_episodes']} episodes):")
    print(f"  Final Success Rate: {summary['training']['final_success_rate']:.2%}")
    print(f"  Final Avg Reward: {summary['training']['final_avg_reward']:.2f}")
    print(f"  Final Avg Episode Length: {summary['training']['final_avg_episode_length']:.1f}")

    print(f"\nEVALUATION ({summary['evaluation']['n_scenarios']} scenarios):")
    print(f"  Success Rate: {summary['evaluation']['success_rate']:.2%}")
    print(f"  Successes: {summary['evaluation']['n_successes']}/{summary['evaluation']['n_scenarios']}")
    print(f"  Avg Reward: {summary['evaluation']['avg_reward']:.2f}")
    print(f"  Avg Episode Length: {summary['evaluation']['avg_episode_length']:.1f}")
    print(f"  Avg Travel Time (successful): {summary['evaluation']['avg_travel_time_successful']:.1f}")
