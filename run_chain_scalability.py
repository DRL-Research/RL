...
def generate_chain_scenario(
    ...
    for i in range(n_int):
        ...
        # Third car: a FULL-CORRIDOR horizontal through-car (the perpendicular crossing).
        # Generalises the strong 2-node recipe to every scale so that EACH node hosts a
        # genuine dense bidirectional crossing — the only regime where the master adds
        # real value (zero_master must struggle, i.e. stay well below ~70%).
        #
        #  n == 1 : a single 4-way W->E local crossing.
        #  n >= 2 : every node injects a horizontal traverser that spans the WHOLE
        #           corridor. The first node drives EAST to the far-east exit and the
        #           last node drives WEST to the far-west exit (exactly the validated
        #           2-node case); interior nodes alternate direction by parity. Because
        #           every traverser runs end-to-end, EACH node is crossed by both an
        #           eastbound and a westbound stream on top of its own S->N / N->S pair —
        #           a dense 4-way conflict at every intersection, at any scale.
        #
        # Edge nodes spawn the traverser from their real W/E approach; interior nodes have
        # no physical W/E lane (those are connectors), so they spawn from the vertical
        # approach and turn onto the main road, staggered ~35 m back to avoid the vertical
        # car's spawn cell.
        if n_int == 1:
            agents.append((_approach(i, 1), "I0_o3", _jit()))           # W -> E (4-way)
        elif i == 0:
            agents.append((_approach(0, 1), far_east, _jit()))          # eastbound (real W appr)
        elif is_last:
            agents.append((_approach(n_int - 1, 3), far_west, _jit()))  # westbound (real E appr)
        elif i % 2 == 1:
            agents.append((_approach(i, 0), far_east, _jit() - 35.0))   # eastbound (S appr, turn E)
        else:
            agents.append((_approach(i, 2), far_west, _jit() - 35.0))   # westbound (N appr, turn W)
...
def global_master_embedding(master_model, lm_embs, proto_exp, deterministic):
    """Recursive 5-way aggregation matching the master's 5 input slots (and the
    parallel suite). Up to 5 sub-masters are combined by ONE shared master node into
    an intermediate embedding; intermediate masters are themselves grouped in 5s, all
    the way up to a single global master. This is the scalability mechanism: every
    group of <=5 masters is managed by one master a level above.
    """
    num_slots = 5
    cur = [np.asarray(e, dtype=np.float32).reshape(-1) for e in lm_embs]
    if not cur:
        return np.zeros(proto_exp.embedding_dim, dtype=np.float32)
    # Intermediate levels: while more than one master's worth of children remain,
    # aggregate in chunks of <=5 (all chunks at a level run in ONE batched forward).
    while len(cur) > num_slots:
        chunk_inputs = [
            _pack_gm_input(cur[i:i + num_slots], proto_exp)
            for i in range(0, len(cur), num_slots)
        ]
        cur = master_embeddings_batch(master_model, chunk_inputs, proto_exp, deterministic)
    if len(cur) == 1:
        return cur[0]
    gm_in = _pack_gm_input(cur, proto_exp)
