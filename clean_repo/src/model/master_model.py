...
class MasterModel:
    ...
    def __init__(self, embedding_size=4, experiment=None, observation_dim=None, **kwargs):
        ...
        elif experiment is not None and getattr(experiment, "MASTER_OBS_DIM", None) is not None:
            # Hierarchical master: NUM_MASTER_SLOTS × (AGENT_STATE_SIZE + type bit) — see Experiment
            self.observation_dim = int(experiment.MASTER_OBS_DIM)
        elif experiment is not None:
            # Legacy flat packing: one 4D kinematic block per controlled vehicle
        ...
        if MasterModel.LOG_STD_INIT is not None:
            policy_kwargs["log_std_init"] = float(MasterModel.LOG_STD_INIT)

        # PURE SB3 PPO - NOTHING CUSTOM
        self.model = PPO(
            "MlpPolicy",
            env,
            learning_rate=getattr(experiment, "MASTER_LR", 1e-4) if experiment is not None else 1e-4,
            n_steps=n_steps,
            batch_size=128,
            gamma=getattr(experiment, "GAMMA", 0.99) if experiment is not None else 0.99,
            gae_lambda=getattr(experiment, "GAE_LAMBDA", 0.95) if experiment is not None else 0.95,
            clip_range=getattr(experiment, "CLIP_RANGE", 0.2) if experiment is not None else 0.2,
            ent_coef=getattr(experiment, "ENT_COEF", 0.01) if experiment is not None else 0.01,
            vf_coef=getattr(experiment, "VF_COEF", 0.5) if experiment is not None else 0.5,
    ...
    def get_proto_action(self, state_tensor):
        ...
        if MasterModel.NORMALIZE_INPUTS:
            obs = obs.astype(np.float32, copy=True)
            for start in range(0, min(len(obs), self.observation_dim), 5):
                obs[start:start + 2] = obs[start:start + 2] / MasterModel.POSITION_SCALE
                obs[start + 2:start + 4] = obs[start + 2:start + 4] / MasterModel.SPEED_SCALE
        ...
        with torch.no_grad():
            ...
            action_np, _ = self.model.predict(
                obs,
                deterministic=bool(self.is_frozen or MasterModel.DETERMINISTIC_PROTO),
            )
            action_np = np.asarray(action_np).reshape(-1)
            action_np = np.nan_to_num(action_np, nan=0.0, posinf=0.0, neginf=0.0).astype(np.float32)
    ...
    @staticmethod
    def _custom_params_paths(path: str) -> tuple[str, str]:
        """
        Return (short_sidecar, legacy_long_suffix) paths for custom master metadata.

        Legacy saved to path + '_custom_params.pt', which can exceed Windows
        MAX_PATH when path already ends with '_master.pth'. The short sidecar
        uses a fixed filename in the same directory as the checkpoint.
        """
        abspath = os.path.abspath(path)
        parent = os.path.dirname(abspath)
        short = os.path.join(parent, "master_custom_params.pt")
        legacy = f"{path}_custom_params.pt"
        return short, legacy
    ...
    def save(self, path):
        ...
        custom_params = {
            ...
            "normalize_inputs": bool(MasterModel.NORMALIZE_INPUTS),
            "position_scale": float(MasterModel.POSITION_SCALE),
            "speed_scale": float(MasterModel.SPEED_SCALE),
        }
        short_path, legacy_path = self._custom_params_paths(path)
        torch.save(custom_params, short_path)
        try:
    ...
    def load(self, path):
        ...
        short_path, legacy_path = self._custom_params_paths(path)
        cp_path = short_path if os.path.exists(short_path) else legacy_path
        if os.path.exists(cp_path):
            custom_params = torch.load(cp_path)
            self.embedding_size = custom_params["embedding_size"]
            if "observation_dim" in custom_params:
                self.observation_dim = int(custom_params["observation_dim"])
            if "normalize_inputs" in custom_params:
                MasterModel.NORMALIZE_INPUTS = bool(custom_params["normalize_inputs"])
            if "position_scale" in custom_params:
                MasterModel.POSITION_SCALE = float(custom_params["position_scale"])
            if "speed_scale" in custom_params:
                MasterModel.SPEED_SCALE = float(custom_params["speed_scale"])
