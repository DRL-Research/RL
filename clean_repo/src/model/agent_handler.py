...
def _resolve_highway_env_id(experiment) -> str:
    """Pick RELintersection / RELroundabout / RELdouble-intersection from Experiment."""
    eid = getattr(experiment, "ENV_ID", None)
    if isinstance(eid, str) and eid.strip():
        return eid.strip()
    # Legacy callers may only swap CONFIG without setting ENV_ID.
    cc = (getattr(experiment, "CONFIG", None) or {}).get("controlled_cars") or {}
    first = next(iter(cc.values()), None)
    sl = (first or {}).get("start_lane")
    if isinstance(sl, tuple) and sl and isinstance(sl[0], str):
        if sl[0].startswith(("A_", "B_")):
            return "RELdouble-intersection-v0"
    return "RELintersection-v0"
...
class Driver(gym.Env):
    ...
    def __init__(self, experiment, master_model=None):
        ...
        gym_env_id = _resolve_highway_env_id(experiment)

        # Create the underlying Highway environment
        self.highway_env = gym.make(
            gym_env_id, render_mode=experiment.RENDER_MODE, config=self.config
        )
    ...
    def reset(self, **kwargs):
        ...
        self.current_state = current_state  # store the prepared (N×4) state, same format as step()
