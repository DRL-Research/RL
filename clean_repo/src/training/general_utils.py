...
def sanitize_float_vector(arr, *, dim: int | None = None) -> np.ndarray:
    """Replace non-finite values and clamp; optional fixed length for agent obs."""
    ...
    if dim is not None:
        ...
        if v.size > d:
            ...
            v = np.pad(v, (0, d - v.size), dtype=np.float32)
...
def get_agent_values_from_observation(car_observation, car_action_array, agent_model):
    with torch.no_grad():
        ...
        obs = sanitize_float_vector(car_observation, dim=odim)
