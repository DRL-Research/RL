...
class RL_2_local:
    ...
    def step_only_local(self, airsim_client, steps_counter):
        ...
        action1, action2 = self.sample_action_by_epsilon_greedy()
        ...
        current_controls1 = airsim_client.getCarControls("Car1")
        updated_controls1 = self.action_to_controls(current_controls1, action1)

        current_controls2 = airsim_client.getCarControls("Car2")
        updated_controls2 = self.action_to_controls(current_controls2, action2)
        return collision, reached_target, updated_controls1, updated_controls2, reward
    ...
    def sample_action_by_epsilon_greedy(self):
        if np.random.binomial(1, p=self.epsilon):
            ...
            action_selected_1 = self.local_network.predict(self.c1_state, verbose=self.verbose).argmax()
            action_selected_2 = self.local_network.predict(self.c2_state, verbose=self.verbose).argmax()
            # print(action_selected)
            return action_selected_1, action_selected_2
