...
def run_episode(experiment, total_steps, env, master_model, agent_model, train_both, training_master):
    ...
    while not done and not truncated:
        ...
        # Agent actions — car_observations already contain [car_state || embedding] from
        # Driver.reset() / Driver.step(), so they are the true obs the agent sees at this step.
        ...
        # Values / log-probs for agent: use the same car_observations that drove the action
        # selection — this guarantees the buffer stores obs consistent with the action taken.
