...
def run_episode(p_master_loss, p_agent_loss, experiment, total_steps, env, master_model, agent_model, agent, training_master):
    ...
    while not done:
        ...
        master_input = torch.tensor([state_car1.tolist() + state_car2.tolist()], dtype=torch.float32)
        ...
        agent_action = agent.get_action(
            agent_model, state_car1_with_embedding, total_steps,
...
# Training loop
# --------------------------------------------------------
def training_loop(p_agent_loss, p_master_loss, p_episode_counter, experiment, env, agent_model, master_model, agent, experiment_config):
    ...
    for cycle in range(experiment.CYCLES):
        ...
        if cycle == 0:
            ...
            for episode in range(experiment.EPISODES_PER_CYCLE):
                ...
                episode_rewards, episode_actions, steps, p_agent_loss, p_master_loss, episode_states = run_episode(
                    p_master_loss, p_agent_loss, experiment, total_steps, env,
                    master_model, agent_model, agent, training_master=True
                )
                print("actions:")
                ...
                if episode_counter % experiment_config.EPISODE_INTERVAL_FOR_TRAINING == 0:
                    pause_experiment_simulation(env)
                    master_model.train_master(master_training_steps)
                    master_model.airsim_manager.pause_simulation()
                    print("Training agent module started")
                    agent_model.learn(total_timesteps=agent_training_steps, log_interval=1)
                    print("Training agent module finished")
                    master_model.airsim_manager.resume_simulation()
                    agent_training_steps = 0
                    master_training_steps = 0
        ...
        if cycle % 2 == 1:
            ...
            for episode in range(experiment.EPISODES_PER_CYCLE):
                ...
                if episode % (experiment_config.EPISODE_INTERVAL_FOR_TRAINING + 1) == 0:
                    master_model.airsim_manager.pause_simulation()
                    print("Training agent module started")
                    agent_model.learn(total_timesteps=agent_training_steps, log_interval=1)
                    print("Training agent module finished")
...
def run_experiment(experiment_config):
    ...
    agent_logger = configure(experiment_config.AGENT_LOGGER_PATH, ["stdout", "csv", "tensorboard"])
    master_logger = configure(experiment_config.MASTER_LOGGER_PATH, ["stdout", "csv", "tensorboard"])
    ...
    p_episode_counter, p_agent_loss, p_master_loss, agent_model, collision_counter, all_rewards, all_actions = training_loop(
        p_agent_loss, p_master_loss, p_episode_counter, experiment_config, env,
        agent_model, master_model, agent, experiment_config
    )

    if not experiment_config.ONLY_INFERENCE:

        plot_results(path=experiment_config.AGENT_LOGGER_PATH, all_rewards=all_rewards, all_actions=all_actions)
        plot_results(path=experiment_config.MASTER_LOGGER_PATH, all_rewards=all_rewards, all_actions=all_actions)
...
#     # run training loop
#     p_episode_counter, p_agent_loss, p_master_loss, agent_model, collision_counter, all_rewards, all_actions = training_loop(
#         p_agent_loss=p_agent_loss,
#         p_master_loss=p_master_loss,
#         p_episode_counter=p_episode_counter,
